...
class SendGridWebhookController extends Controller
{
    public function __construct(
        ...
        protected SendGridDriver $sendGridDriver,
    ) {}

    public function __invoke(Request $request): JsonResponse
    {
        $payload = $request->all();
        $parsed = $this->sendGridDriver->parseWebhookPayload($payload);
        $parsedEmail = $this->emailParser->parse($parsed);

        $recipient = $parsed['to_addresses'][0] ?? null;
        $channel = $recipient
            ? EmailChannel::where('email_address', $recipient)->where('driver', 'sendgrid')->first()
            : null;
